...
export async function getProducts({ category, search, sortBy, pageSize = 12, lastDoc } = {}) {
  try {
    ...
    // Apply limit locally if we had to remove orderBy
    if (pageSize) {
...
export async function getAllUsers() {
  try {
    ...
    users.sort((a, b) => {
      let dateA = 0;
      if (a.createdAt) {
        if (typeof a.createdAt.toMillis === 'function') dateA = a.createdAt.toMillis();
        else if (a.createdAt.seconds) dateA = a.createdAt.seconds * 1000;
        else if (a.createdAt instanceof Date) dateA = a.createdAt.getTime();
        else if (typeof a.createdAt === 'string' || typeof a.createdAt === 'number') dateA = new Date(a.createdAt).getTime();
      }
      
      let dateB = 0;
      if (b.createdAt) {
        if (typeof b.createdAt.toMillis === 'function') dateB = b.createdAt.toMillis();
        else if (b.createdAt.seconds) dateB = b.createdAt.seconds * 1000;
        else if (b.createdAt instanceof Date) dateB = b.createdAt.getTime();
        else if (typeof b.createdAt === 'string' || typeof b.createdAt === 'number') dateB = new Date(b.createdAt).getTime();
      }
